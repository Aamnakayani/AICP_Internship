...
// Global variable to keep track of daily total payments
...
double calculatePrice(string day, int arrivalHour, int hours, string frequentParkingNumber) {
    ...
    // Determine price per hour, max stay, and discount based on day and arrival time
    if (day == "Sunday" && arrivalHour >= 8 && arrivalHour <= 15) {
        ...
        pricePerHour = 2.00; // Evening rate
        maxStay = 2;
    }
    ...
    // Apply discount if frequent parking number is correct
...
// Task 2
void task2() {
    dailyTotal = 0.0; // Reset daily total
    int numCustomers;
